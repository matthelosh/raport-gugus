       ...
       <ul  class = "nav">
          <li     class = "nav-item">
              <a href="javascript:void(0);"class="nav-link">
                  <select name  = "tapel" id = "tapel" style="display:block;width:100%;padding: 3px 20px;text-align:center;background: #ffffff;border:1px solid #333;">
                    <option value = "0">Pilih Tapel</option>
                    @php
                      $th = date('Y');
                      for($i = $th-5;$i<$th+5;$i++){   
                        $selected = ($i == $th) ? 'selected': '';
                        echo "<option value='".$i.'_'.($i+1)."' $selected>".$i.'/'.($i+1)."<span class = 'caret'></span></option>";
                      }
                    @endphp
            ...
            <li     class = "nav-item">
              <a href="javascript:void(0);"class="nav-link">
                  <select name  = "semester" id = "semester" style="display:block;width:100%;padding: 3px 20px;text-align:center;background: #ffffff;border:1px solid #333;">
                    <option value = "0">Pilih Semester</option>
                    @php
                      $bulan = date('m');
                        $ganjil = ($bulan > 6) ? 'selected': '';
                        $genap = ($bulan < 7) ? 'selected': '';
                        echo "<option value='ganjil' $ganjil>Ganjil</option><option value='genap' $genap>Genap</option>";
